...
/**
 * IntArrayPool is a source and repository of <code>byte[]</code> objects. Its purpose is to
...
public class IntArrayPool {
    ...
    private List<int[]> mBuffersByLastUse = new LinkedList<int[]>();
    private List<int[]> mBuffersBySize = new ArrayList<int[]>(64);
    ...
    protected static final Comparator<int[]> BUF_COMPARATOR = new Comparator<int[]>() {
        @Override
     ...
     */
    public IntArrayPool(int sizeLimit) {
     ...
     */
    public synchronized int[] getBuf(int len) {
        for (int i = 0; i < mBuffersBySize.size(); i++) {
            int[] buf = mBuffersBySize.get(i);
            if (buf.length >= len) {
    ...
    private synchronized void trim() {
        while (mCurrentSize > mSizeLimit) {
            int[] buf = mBuffersByLastUse.remove(0);
